    ...
    def get_overtime(self,day):
        ...
        if self:
            ...
            if we_ot_id:
                ...
                res['we_rate'] = 0
                res['we_after'] = 0
                for line in we_ot_id.line_ids:
                    if int(line.day) == weekday:
                        res['we_rate'] = line.rate
                        res['we_after'] = line.active_after
                        break
            else:
                res['we_rate'] = 1
                res['we_after'] = 0
    ...
    def get_late(self, period, cnt):
        ...
        if self.late_rule_id:
            time_ids = self.late_rule_id.line_ids.sorted(
                key=lambda r: r.time)
            # raise ValidationError([l.time for l in time_ids] + [period])
...
class HrOvertimeRule(models.Model):
    ...
    active_after = fields.Float(string="Apply after",
                                help="After this time the overtime will be calculated")
    rate = fields.Float(string='Rate')
    line_ids = fields.One2many('hr.overtime.rule.line.day','overtime_id')

class HrOvertimeRule(models.Model):
    _name = 'hr.overtime.rule.line.day'
    overtime_id = fields.Many2one('hr.overtime.rule')
