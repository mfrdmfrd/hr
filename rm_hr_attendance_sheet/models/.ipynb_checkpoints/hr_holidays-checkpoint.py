...
class HrPublicHoliday(models.Model):
    ...
    company_ids = fields.Many2many('res.company', string='Company',relation="company_id_ph_rel1",
                               column1="company_id_col2",
                               column2="company_id_col3",)
    ...
    def get_employee_ids(self):
        ...
        if self.type_select == 'dep':
            ...
            self.emp_ids = self.env['hr.employee'].search(
                [('id', 'in', emp_ids)])
        elif self.type_select == 'company':
            for employee in self.env['hr.employee'].search([]):
                list1 = self.company_ids.ids
                list2 = employee.company_id.id
                match = any(map(lambda v: v in list1, list2))
                if match:
                    emp_ids.append(employee.id)
            self.emp_ids = self.env['hr.employee'].search(
